PM.localAdd("ChannelThread", {
  state: {
    inbox: Object.create(null)
  },
  open(taskId) {
    if (taskId === undefined) taskId = TS.id()
    if (!this.state.inbox[taskId]) this.state.inbox[taskId] = []
  },
  close(taskId) {
    if (taskId === undefined) taskId = TS.id()
  ...
  send(msg, target) {
    ...
    if (Array.isArray(target)) {
      for (const id of target) { if (inbox[id]) inbox[id].push(msg) }
      return
    }
  ...
  recv(target, number = 1) {
    ...
    const take = (id, n) => {
      ...
      if (n === Infinity) { const all = box.slice(); box.length = 0; return all }
      const out = []
      while (n-- > 0 && box.length) out.push(box.shift())
      return out
    }
    const finish = (items) => number === 1 ? items[0] : items
    if (target === null) {
      if (number === 1) {
        for (const id in inbox) { if (inbox[id]?.length) return inbox[id].shift() }
        return undefined
      }
      const out = []
    ...
    if (Array.isArray(target)) {
      if (number === 1) {
        for (const id of target) { if (inbox[id]?.length) return inbox[id].shift() }
        return undefined
      }
      const out = []
    ...
    if (!inbox[target]) throw new Error(`Task ${target} is not open for messages`)
  ...
  override: {
    // TS._removeTask now instead of TaskScheduler._removeTask
