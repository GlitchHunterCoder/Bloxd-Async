PM.localAdd("CustomPromise", {
  Promise: class {
      ...
      *[Symbol.iterator]() {
        ...
        for (const h of handlers) {
          try {
            if (this.state === "fulfilled") {
              ...
              h.resolveNext(h.onRejected ? h.onRejected(this.value) : undefined)
            }
      ...
      then(onFulfilled, onRejected) {
        return new PM.run("CustomPromise").Promise((resolve, reject) => {
          this.handlers.push({ onFulfilled, onRejected, resolveNext: resolve, rejectNext: reject })
          if (this.state !== "pending") TS.add(() => this.runHandlers(), 1)
        })
      }
    ...
    create(executor) { return new this.PromiseImpl(executor) }
    resolve(v)       { return this.create(r => r(v)) }
    reject(e)        { return this.create((_, r) => r(e)) }
    all(...promises) {
